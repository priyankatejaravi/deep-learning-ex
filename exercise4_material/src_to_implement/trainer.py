    ...
    def val_test(self):
        ...
        with t.no_grad():
            for x, y in self._val_test_dl:
                if self._cuda:
                    x, y = x.cuda(), y.cuda()
                loss, out = self.val_test_step(x, y)
                running_loss += loss.item()
                preds.append((out >= 0.5).cpu().float())
                targets.append(y.cpu())
        score = f1_score(t.cat(targets).numpy(), t.cat(preds).numpy(), average='macro')
        print('Val F1: {:.4f}'.format(score))
    ...
    def fit(self, epochs=-1):
        ...
        while True:
            ...
            if self._early_stopping_patience > 0:
                no_improve = 0 if v_loss < best else no_improve + 1
                best = min(best, v_loss)
